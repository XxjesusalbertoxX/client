...
@Component({
  standalone: true,
  selector: 'app-game-anfitrion',
  imports: [CommonModule, LoteriaCardComponent],
  templateUrl: './game-anfitrion.component.html',
  styleUrls: ['./game-anfitrion.component.scss']
})
export class GameAnfitrionComponent implements OnInit {
  ...
  drawCard() {
    ...
    if (!gameId || this.isDrawingCard) return;

    this.isDrawingCard = true;

    this.loteriaService.drawCard(gameId).subscribe({
      next: (response) => {
        this.toastr.success(`¡Carta sacada: ${this.formatCardName(response.drawnCard)}!`);
        this.isDrawingCard = false;
      },
      error: (error) => {
        this.toastr.error(error.error?.message || 'Error al sacar carta');
        this.isDrawingCard = false;
      }
    });
  }
  ...
  roundPercentage(value: number, total: number): number {
  ...
  endGame() {
    ...
    if (confirm('¿Estás seguro de terminar la partida?')) {
      // Usar leaveGame ya que no hay endpoint endGame específico
      this.loteriaService.leaveGame(gameId).subscribe({
        next: (response) => {
          this.toastr.info('Partida terminada');
        },
        error: (error) => {
          this.toastr.error(error.error?.message || 'Error al terminar partida');
        }
      });
    }
  }
  ...
  getFichaImagePath(): string {
    return '/assets/Ficha.png'; // Ruta correcta
  }
  ...
  get canDrawCard(): boolean {
    return this.viewModel.cardsRemaining() > 0 && !this.isDrawingCard;
  }

  get playersWithCards() {
    // Usar hostView para obtener las cartas de los jugadores
    const hostView = this.viewModel.hostView();
    return hostView ? hostView.playersCards : [];
  }

  getPlayerGridClass(): string {
    const playerCount = this.playersWithCards.length;

    if (playerCount <= 2) return 'grid-cols-1 md:grid-cols-2';
    if (playerCount <= 4) return 'grid-cols-1 md:grid-cols-2 lg:grid-cols-2';
